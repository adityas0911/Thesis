...
from source.agents.random_agent import RandomWrapper
...
def evaluate_random(configuration_path: str = None,
  ...
  if evaluating:
    ...
    if timestamp is None:
      ...
      first_episode_logged = False
    else:
      first_episode_logged = True
  else:
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    first_episode_logged = False
  if training_episode_percentage:
    maximum_training_episode = get_maximum_training_episode(training_data_directory = training_data_directory)
    episodes = int(maximum_training_episode * training_episode_percentage)
  else:
  ...
  model: RandomWrapper = RandomWrapper(vectorized_environment = vectorized_environment,
...
if __name__ == "__main__":
  parser: argparse.ArgumentParser = argparse.ArgumentParser(description = "Evaluate Random agent for all alpha values (AUTO-OPTIMIZED)")
  ...
  evaluate_random(configuration_path = arguments.configuration_path,
